...
# Initialisation de la mémoire d'état de session (Anti-bug)
...
# Dictionnaire de traduction intégré pour un affichage pro
lang = {
...
def verifier_si_utilisateur_existe(email):
    ...
    try:
        ...
        if res.status_code == 200 and len(res.json()) > 0:
            return res.json()[0]  # Renvoie le profil de l'artisan trouvé
    except:
        pass
    return None
...
def extraire_leads_strict(metier, ville):
    # FILTRE UNIQUE STRICT : Même métier ET (Même ville OU tag global mondial)
...
if not st.session_state.authentifie:
    ...
    if email_input:
        ...
        if utilisateur:
            # COMPTE EXISTANT -> Connexion automatique sans réinscription
            ...
            # COMPTE INEXISTANT -> Formulaire d'inscription unique
                ...
                if st.form_submit_button("🚀 Créer mon compte unique et ouvrir le flux"):
                    if choix_ville:
                        if inscrire_nouvel_artisan(email_input, choix_metier, choix_ville):
                            ...
                            st.error("Erreur technique de communication avec la base de données.")
                    else:
                        st.error("Veuillez remplir le champ de la ville.")

# ==========================================
# 4. LE TABLEAU DE BORD STRICT PRIVÉ LOCALISÉ
# ==========================================
else:
    ...
    if leads:
        ...
        for idx, client in enumerate(leads):
            with st.container(border=True):
                ...
                # ROUTAGE COMMERCIAL INTELLIGENT WHATSAPP VS LIEN DIRECT
                ...
                # ENVOI AUTOMATIQUE PAR EMAIL (RESEND) - PARFAITEMENT ALIGNÉ
                if st.button(f"📧 Recevoir la fiche client par E-mail (Lead #{idx})", key=f"resend_{idx}", use_container_width=True):
                    ...
                    payload_resend = {
                        ...
                        "html": f"<h3>🚀 ZELIA GLOBAL</h3><p>Un client cherche un professionnel dans votre secteur :</p><p><strong>Détails :</strong> {client.get('texte')}</p><p><a href='{lien_brut}'>Répondre immédiatement au client</a></p>"
                    }
                    try:
                        ...
                        if res.status_code == 200 or res.status_code == 201:
                            ...
                            st.error(f"Refus du serveur d'e-mail (Code {res.status_code})")
                    except Exception as e:
                        st.error(f"Connexion au serveur e-mail interrompue : {e}")
    else:
        st.warning(f"🔎 Aucun client trouvé à {st.session_state.user_ville.upper()} pour le métier de {st.session_state.user_metier} à cet instant. Le robot continue ses recherches.")
