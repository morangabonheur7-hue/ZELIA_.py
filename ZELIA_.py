    ...
    if email_input:
        ...
        if utilisateur:
            ...
            if st.button("🔓 Ouvrir mon tableau de bord", use_container_width=True, type="primary"):
                st.session_state.user_email, st.session_state.user_metier, st.session_state.user_ville = utilisateur['email'], utilisateur['metier'], utilisateur['ville']
            ...
            with st.form("form_inscription"):
                ...
                if st.form_submit_button("🚀 Créer mon compte"):
                    ...
                    else: st.error("Veuillez indiquer votre ville.")
            ...
            for idx, client in enumerate(leads_filtres):
                with st.container(border=True):
                    ...
                    if num_client:
                        ...
                        st.link_button("➡️ Ouvrir le site d'origine pour répondre", lien_brut, use_container_width=True)
                    ...
                    if st.button(f"📧 Recevoir la fiche par E-mail", key=f"resend_{idx}", use_container_width=True):
                        ...
                        payload_resend = {"from": "Zelia Global <onboarding@resend.dev>", "to": [st.session_state.user_email], "subject": f"🚨 NOUVEAU CHANTIER", "html": f"<p>{client.get('texte')}</p><br><a href='{lien_brut}'>Lien</a>"}
                        try:
                            ...
                            else: st.error(f"Erreur serveur ({res.status_code})")
                        except: st.error("Connexion échouée.")
        else: st.info("🔎 Aucun chantier dans cette tranche. Élargissez le filtre temporel.")
    else: st.warning("🔎 Aucun chantier trouvé. Le robot scanne le web en continu.")
...
c1, c2 = st.columns(2)
with c1: st.link_button("💬 Support WhatsApp Direct", f"https://wa.me242055967601?text=Bonjour Support Zelia", use_container_width=True)
