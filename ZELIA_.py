...
res         = requests.get ( url, headers=headers, timeout= 5 )
        si res.status_code == 200 :​
            ...
            si  len ( donnees ) > 0 :
                u = donnees [ 0 ]
                si  "statut_abonnement"  n'est pas  dans u ou u [ "statut_abonnement" ]  vaut  Aucun : u [ "statut_abonnement" ] = "inactif"
                retour u
    sauf : passer
    renvoyer  Aucun

def  inscrire_nouvel_artisan ( email, métier, ville ) :
    url = f" { SUPABASE_URL } /rest/v1/utilisateurs"
    headers = { "apikey" : SUPABASE_KEY, "Authorization" : "Bearer { SUPABASE_KEY } " , "Content-Type" : "application/json" , "Prefer" : "return=minimal" }
    charge utile = [ {
        "email" : email.minuscule ( ) ,
        "métier" : métier. inférieur ( ) ,
        "ville" : ville. inférieur ( ) ,
        "statut_abonnement" : "inactif"
    } ]
    essayer :
res         = requests.post ( url, json=payload, headers=headers, timeout= 5 )
        si res.status_code est : retourner True​  
    sauf : passer
    renvoyer  Faux

def  particulier_deposer_chantier ( métier, ville, description, téléphone ) :
    ...
    charge utile = [ {
        ...
        "ville" : ville. inférieur ( ) ,
        "texte" : texte_final,
        "téléphone" : téléphone.strip ( ) ,
        "lien" : "https://streamlit.app" ,
        "plateforme" : "Zélia Public Direct"
    } ]
    essayer :
res         = requests.post ( url, json=payload, headers=headers, timeout= 5 )
        si res.status_code est : retourner True​  
...
def  extraire_leads_strict ( métier, ville ) :
    ...
    essayer :
res         = requests.get ( url, headers=headers, timeout= 5 )
        si res.status_code == 200 : retourner res.json ( )​​
    sauf : passer
    retour  [ ]
